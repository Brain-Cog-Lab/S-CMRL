...
def class_contrastive_loss(feature_1, feature_2, label, batch_size, temperature=0.01):
    ...
    #     # (BS, BS)
    #     score = torch.mm(feature_1[t], feature_2.transpose(0, 1)) / temperature
    ...
    if args.load_avmodel:
        if args.dataset == "CREMAD":
            checkpoint = torch.load(
                "/mnt/home/hexiang/S-CMRL/SNN/exp_results/AVspikformer-CREMAD-audio-visual-interaction-Add-attn-method_SpatialTemporal-cross-attn_True-alpha_1.5-contrastive-True-temperature_0.07-snr_-100-embed_dims_256-LIFNode-4/model_best.pth.tar",
        ...
        audio_patch_embed_state_dict = {k.replace('audio_patch_embed.', ''): v
                                         ...
                                         if k.startswith('audio_patch_embed.')}

        visual_patch_embed_state_dict = {k.replace('visual_patch_embed.', ''): v
                                         for k, v in checkpoint.items()
                                         if k.startswith('visual_patch_embed.')}

        audio_mlp = {k.replace('mlp.0.mlp.', ''): v
                                         for k, v in checkpoint.items()
                                         if k.startswith('mlp.0.mlp.')}

        visual_mlp = {k.replace('mlp.1.mlp.', ''): v
                                         for k, v in checkpoint.items()
                                         if k.startswith('mlp.1.mlp.')}
        if args.modality == "audio":
            model.patch_embed.load_state_dict(audio_patch_embed_state_dict, strict=True)
            model.block[0].mlp.load_state_dict(audio_mlp, strict=True)
        else:
            model.patch_embed.load_state_dict(visual_patch_embed_state_dict, strict=True)
            model.block[0].mlp.load_state_dict(visual_mlp, strict=True)
