...
import os
import sys
sys.path.append("/mnt/home/hexiang/BMCF/")
...
def main(incremental_step, old_model, model, exemplar_loader_train, loader_train, loader_eval, output_dir):
    ...
    if args.local_rank == 0:
        ...
        save_dir = os.path.join(output_dir, "step_{}".format(incremental_step))
    ...
    try:  # train the model
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            train_metrics = train_epoch(
                incremental_step, epoch, old_model, model, exemplar_loader_train, loader_train, optimizer, train_loss_fn, args,
...
def train_epoch(
        incremental_step, epoch, old_model, model, exemplar_loader, loader, optimizer, loss_fn, args,
    ...
    if incremental_step == 0:
        ...
        loader = zip(loader, cycle(exemplar_loader))
    ...
    for batch_idx, samples in enumerate(loader):
        if incremental_step == 0:
            ...
            if not args.prefetcher or args.dataset != 'imnet':
                if args.modality == "audio-visual":
                    inputs, target = tuple(item.type(torch.FloatTensor).cuda() for item in inputs), target.cuda()
                else:
            ...
            if args.dataset == "UrbanSound8K" or args.dataset == "AvCifar10" or args.dataset == "CREMAD":
                if args.modality == "audio-visual":
                    ...
                    inputs = repeat(inputs, 'b c w h -> b t c w h', t=args.step)
                    inputs_old = repeat(inputs_old, 'b c w h -> b t c w h', t=args.step)
            last_batch = batch_idx == last_idx
            ...
            if not args.prefetcher or args.dataset != 'imnet':
                if args.modality == "audio-visual":
                    inputs, target = tuple(item.type(torch.FloatTensor).cuda() for item in inputs), target.cuda()
                    inputs_old, target_old = tuple(item.type(torch.FloatTensor).cuda() for item in inputs_old), target_old.cuda()
                    inputs = tuple(torch.cat((item1, item2), dim=0) for item1, item2 in zip(inputs, inputs_old))
                else:
                    inputs, target = inputs.type(torch.FloatTensor).cuda(), target.cuda()
            ...
            acc_target = torch.cat((target, target_old), dim=0)
            target = target % args.class_num_per_step

            curr_out = output[:data_batch_size, last_step_out_class_num:]
            loss_curr = loss_fn(curr_out, target)

            prev_out = output[data_batch_size:data_batch_size + exemplar_data_batch_size, :last_step_out_class_num]
            loss_prev = loss_fn(prev_out, target_old)

            loss = (loss_curr * data_batch_size + loss_prev * exemplar_data_batch_size) / (
                    data_batch_size + exemplar_data_batch_size)

            loss_KD = torch.zeros(incremental_step).cuda()
            T = 2

            for t in range(incremental_step):
...
def validate_incremental_step(args, model, loader, step, amp_autocast=suppress,
    ...
    with torch.no_grad():
        ...
        if step > 0:
            ...
            _logger.info('forgetting: {:.6f}'.format(forgetting))
...
if __name__ == '__main__':
    ...
    total_incremental_steps = args.num_classes // args.class_num_per_step  # 10 // 2
    for incremental_step in range(total_incremental_steps):
        _logger.info('----------------Incremental step: {}-----------------'.format(incremental_step))

        # now config only for imnet
        data_config = resolve_data_config(vars(args), model=model, verbose=False)
        loader_train, loader_eval, mixup_active, mixup_fn = eval('get_%s_CL_data' % args.dataset)(
        ...
        model = create_model(
            ...
            num_classes=max(args.class_num_per_step, args.class_num_per_step * incremental_step),
        ...
        if incremental_step > 0:
            model.load_state_dict(torch.load(
                os.path.join(output_dir, "step_{}/model_best.pth.tar".format(incremental_step-1)), map_location='cpu')['state_dict'])
            old_model = deepcopy(model)
            step_out_class_num = (incremental_step + 1) * args.class_num_per_step
            model.incremental_classifier(step_out_class_num)

        exemplar_loader_train, _, _, _ = eval('get_%s_CL_exemplar_data' % args.dataset)(
            batch_size=args.batch_size,
            step=args.step,
            args=args,
            _logge=_logger,
        ...
        main(incremental_step, old_model, model, exemplar_loader_train, loader_train, loader_eval, output_dir)

        step_accuracy, step_forgetting = validate_incremental_step(args, model, loader_eval, incremental_step)
    ...
    _logger.info('Average Accuracy: {:.6f}'.format(Mean_accuracy))
    Mean_forgetting = np.mean(step_forgetting_list)
    _logger.info('Average Forgetting: {:.6f}'.format(Mean_forgetting))