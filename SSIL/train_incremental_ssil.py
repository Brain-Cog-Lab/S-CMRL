...
def train(args, step, train_data_set, val_data_set, exemplar_set):
    ...
    if step == 0:
        ...
        if args.e_prompt:
            ...
            # for i in range(step):  # 把之前的prompt冻住
            #     model.a_prompt.prompt[i].requires_grad = False  # 设置为 False
            #     model.a_prompt.prompt_key[i].requires_grad = False  # 设置为 False
            #     model.v_prompt.prompt[i].requires_grad = False  # 设置为 False
            #     model.v_prompt.prompt_key[i].requires_grad = False  # 设置为 False
    ...
    if args.e_prompt:
        model.a_prompt.to_cuda(device)
        model.v_prompt.to_cuda(device)
        ...
        for samples in iterator:
            if step == 0:
                ...
                if args.modality == 'visual':
                    ...
                    if args.e_prompt:
                        task_ids = torch.div(total_labels, args.class_num_per_step, rounding_mode='trunc').unsqueeze(1)
                    ...
                    if args.e_prompt:
                        out, out_for_kl, a_sim, v_sim = model(visual=total_visual, audio=total_audio, task_id=task_ids, task_ids_for_kl=task_ids_for_kl)
                    else:
                        out = model(visual=total_visual, audio=total_audio)
                    with torch.no_grad():
                        if args.e_prompt:
                            old_out, a_sim_old, v_sim_old = old_model(visual=total_visual, audio=total_audio, task_id=task_ids_for_kl)
                ...
                for t in range(step):
                    ...
                    output_log = F.log_softmax(out_for_kl[:, start:end] / T, dim=1)
